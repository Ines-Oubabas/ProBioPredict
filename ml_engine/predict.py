...
model_path = os.path.join(os.path.dirname(__file__), "modele.tflite")
...
def predict_from_sequence(sequence):
    """Prédire à partir d'une seule séquence ADN"""
...
def predict(input_data):
    """Fonction principale : accepte une séquence ou un CSV"""
    if isinstance(input_data, str):
        cleaned = input_data.strip().upper()

        if len(cleaned) == 1000 and set(cleaned).issubset({"A", "T", "G", "C"}):
            return predict_from_sequence(cleaned)
